...
# Cache pour le mapping des catégories
_category_mapping_cache = None

def load_category_mapping():
    """Charge le mapping des catégories vers leurs chemins réels"""
    ...
    if not os.path.exists(CATEGORY_MAPPING_FILE):
        print("⚠️ Fichier de mapping non trouvé, utilisation des noms directs")
        return {}
    
    try:
        with open(CATEGORY_MAPPING_FILE, "r", encoding='utf-8') as f:
            _category_mapping_cache = json.load(f)
            print(f"✅ Mapping chargé: {len(_category_mapping_cache)} catégories")
            return _category_mapping_cache
...
def get_category_path(category_name):
    """Retourne le chemin réel d'une catégorie en utilisant le mapping"""
...
def read_file(category, filename):
    ...
    # 3. Define the absolute path for the intended category directory
    category_dir = os.path.abspath(os.path.join(app_root, 'data', category))
...
def open_folder(category):
    ...
    try:
        folder_path = os.path.join(os.getcwd(), 'data', category)
...
def extract_file_creation_hour(category_name, filename):
    ...
    try:
        filepath = f"data/{category_name}/{filename}"
        if not os.path.exists(filepath):
            return None
    ...
    try:
        ...
        for i, (cat_name, cat_data) in enumerate(sorted_categories):
            ...
            for file in cat_data['files']:
                ...
                # Extraire juste le nom du fichier (gérer les sous-dossiers)
                import urllib.parse
                display_filename = file['filename'].split('/')[-1]  # Dernier élément du chemin
                encoded_filename = urllib.parse.quote(file['filename'])  # Encoder pour l'URL

                files_html += f"""
                <div class="file-item">
                    <div class="file-info">
                        <span class="file-emoji">📄</span>
                        <span class="file-name">{display_filename}</span>
                        <span class="file-date">{display_date}</span>
                    </div>
                    <a href="/read/{cat_name}/{encoded_filename}" target="_blank" class="read-btn">Lire</a>
                </div>
                """
...
def fusionner_dossier_backend():
    ...
    try:
        ...
        folder_path = os.path.join(os.getcwd(), 'data', dossier)
