...
def all_notes():
    ...
    <body>
        ...
        <script>
            ...
            // Fonction de recherche améliorée avec API Node.js
            let searchTimeout;
            ...
            // Fonction de recherche utilisant l'API Node.js
            async function performSearch(searchTerm) {
                try {
                    // Afficher un indicateur de chargement
                    searchInput.style.background = 'linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%)';
                    searchInput.placeholder = '🔍 Recherche en cours...';

                    // Faire la requête à l'API de recherche Python
                    const response = await fetch(`/search_files?q=${searchTerm}`, {
                        method: 'GET',
                        headers: {
                    ...
                    if (data.success) {
                        updateSearchResults(data.results, searchTerm);
                    } else {
                    ...
                    showSearchError('Service de recherche non disponible');
                    // Fallback vers la recherche basique si l'API n'est pas disponible
                    performBasicSearch(searchTerm);
                } finally {
                    // Restaurer l'apparence normale
                    searchInput.style.background = '';
                    searchInput.placeholder = '🔍 Rechercher dans les fichiers...';
                }
            }

            // Fonction de recherche basique (fallback)
            function performBasicSearch(searchTerm) {
                ...
                accordionItems.forEach(item => {
                    ...
                    fileItems.forEach(fileItem => {
                        ...
                        if (fileName.includes(searchTermLower) || folderName.includes(searchTermLower)) {
                            fileItem.style.display = 'flex';
                    ...
                    if (hasVisibleFiles || folderName.includes(searchTermLower)) {
                        item.style.display = 'block';
                    } else {
            ...
            function updateSearchResults(results, searchTerm) {
                ...
                // Créer un objet pour mapper les chemins de fichiers aux éléments DOM
                const fileMap = new Map();

                accordionItems.forEach(item => {
                    const fileItems = item.querySelectorAll('.file-item');
                    fileItems.forEach(fileItem => {
                        const readLink = fileItem.querySelector('.read-btn');
                        if (readLink) {
                            const href = readLink.getAttribute('href');
                            // Extraire le chemin du fichier depuis l'URL
                            const urlMatch = href.match(/\/read\/([^\/]+)\/(.+)/);
                            if (urlMatch) {
                ...
                // Afficher seulement les fichiers qui correspondent
                const visibleCategories = new Set();

                results.forEach(result => {
                    const fileInfo = fileMap.get(result.relativePath);
                    if (fileInfo) {
                        fileInfo.item.style.display = 'block';
                        fileInfo.fileItem.style.display = 'flex';
                        visibleCategories.add(fileInfo.category);

                        // Ajouter des informations sur les correspondances
                        const fileInfoDiv = fileInfo.fileItem.querySelector('.file-info');
                        let matchInfo = fileInfo.fileItem.querySelector('.match-info');
                        if (!matchInfo) {
                            matchInfo = document.createElement('div');
                            matchInfo.className = 'match-info';
                            matchInfo.style.fontSize = '0.8rem';
                            matchInfo.style.color = '#28a745';
                            matchInfo.style.marginTop = '5px';
                            fileInfoDiv.appendChild(matchInfo);
                        }
                        matchInfo.textContent = `${result.totalMatches} correspondance(s) trouvée(s)`;
                    }
                });
            ...
            function showNoResults(searchTerm) {
                ...
                noResultsDiv.innerHTML = `
                    ...
                    <p>Aucune correspondance pour "<strong>${searchTerm}</strong>" dans le contenu des fichiers.</p>
                    <p style="font-size: 0.9rem; margin-top: 10px;">
                        Essayez avec des termes différents ou vérifiez l'orthographe.
                    </p>
                `;

                // Masquer tous les éléments d'accordéon
                const accordionItems = document.querySelectorAll('.accordion-item');
                accordionItems.forEach(item => {
                    item.style.display = 'none';
                });
            ...
            // Fonction pour afficher une erreur de recherche
            function showSearchError(message) {
                ...
                errorDiv.innerHTML = `
                    <strong>⚠️ Erreur de recherche:</strong> ${message}
                    <br><small>Utilisation de la recherche basique en attendant.</small>
                `;
