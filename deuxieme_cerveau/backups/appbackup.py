...
def read_file_with_fallback(filepath):
    """Lire un fichier avec plusieurs encodages en fallback"""
    ...
    # Dernier recours: utf-8 avec remplacement des erreurs
    try:
        with open(filepath, 'r', encoding='utf-8', errors='replace') as f:
            return f.read()
    except Exception as e:
        raise Exception(f"Impossible de lire le fichier {filepath}: {e}")
    ...
    try:
        ...
        with open(fusion_filepath, 'w', encoding='utf-8') as fusion_file:
            ...
            for file_data in all_files_data:
                ...
                if os.path.exists(filepath):
                    try:
                        content = read_file_with_fallback(filepath).strip()
                        if content:
